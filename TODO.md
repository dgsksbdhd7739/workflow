...
- [x] 11. Web-Änderungen (Ticket-Feature) waren unter `app.workflow-app.de` nicht sichtbar, weil die Domain per DNS noch auf Vercel zeigt (STRATO-Domain erst ab ca. Anfang Oktober 2026 umstellbar, siehe Agents.md). Root-Ursache geklärt: VPS (`https://217-154-200-72.sslip.io`) hatte den aktuellen Stand bereits korrekt, nur die `.de`-Domain zeigt noch auf den alten Vercel-Build. Kein Bugfix nötig, nur Klarstellung — bis zur DNS-Umstellung über die sslip.io-Adresse testen.
- [x] 12. Release-APK (v1.5.0) mit dem Ticket-Feature gebaut und veröffentlicht, da native Plugin-Änderungen nicht per Live-Update nachziehbar sind. → `android/app/build.gradle` versionCode 6→7, versionName 1.3.12→1.5.0; `npx cap sync android` + `gradlew assembleRelease`; APK nach `landing/assets/workflow.apk` kopiert, Downloadlink in `landing/index.html` auf `WorkFlow-v1.5.0.apk` aktualisiert; Landing per `vercel --prod` neu deployt und unter `https://workflow-app.de` verifiziert (Downloadlink + Datei erreichbar).
