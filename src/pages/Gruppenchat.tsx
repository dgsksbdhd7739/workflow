...
import type { ChatNachricht } from '../types/database'
...
export function Gruppenchat() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`chat-${unternehmenId}`)
      .on(
        'postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'chat_nachrichten', filter: `unternehmen_id=eq.${unternehmenId}` },
        (payload) => {
          setNachrichten((prev) => (prev.some((n) => n.id === payload.new.id) ? prev : [...prev, payload.new as ChatNachricht]))
        },
      )
      .on(
        'postgres_changes',
        { event: 'DELETE', schema: 'public', table: 'chat_nachrichten', filter: `unternehmen_id=eq.${unternehmenId}` },
        (payload) => {
  ...
  const handleSenden = async (e: FormEvent) => {
    ...
    if (!inhalt || !user || !unternehmenId) return
    setSenden(true)
    setFehler(null)
    setText('')
    const { data, error } = await supabase
      .from('chat_nachrichten')
      .insert({ unternehmen_id: unternehmenId, user_id: user.id, text: inhalt })
      .select()
      .single()
    setSenden(false)
    if (error) {
    ...
    // Nicht auf das Realtime-Echo verlassen: kurz nach dem Laden der Seite
    // kann die Subscription das eigene Insert-Event verpassen.
    if (data) setNachrichten((prev) => (prev.some((n) => n.id === data.id) ? prev : [...prev, data as ChatNachricht]))
  }

  const handleLoeschen = async (id: string) => {
    setNachrichten((prev) => prev.filter((n) => n.id !== id))
    const { error } = await supabase.from('chat_nachrichten').delete().eq('id', id)
    if (error) setFehler(error.message)
  }

  return (
    <div className="mx-auto flex h-full max-w-2xl flex-col px-4 py-6">
      <h1 className="mb-4 text-xl font-semibold text-text">Team-Chat</h1>
